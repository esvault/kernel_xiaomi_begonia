...
static void spsa2_powersave_bias_init(struct cpufreq_policy *policy)
{
				    ...
				    const char *buf, size_t count)
{
	...
	list_for_each_entry(policy_dbs, &attr_set->policy_list, list)
		spsa2_powersave_bias_init(policy_dbs->policy);
...
static void od_start(struct cpufreq_policy *policy)
{
	...
	dbs_info->sample_type = OD_NORMAL_SAMPLE;
	spsa2_powersave_bias_init(policy);
}
...
void od_register_powersave_bias_handler_spsa2(unsigned int (*f)
...
EXPORT_SYMBOL_GPL(od_register_powersave_bias_handler_spsa2);

void od_unregister_powersave_bias_handler_spsa2(void)
{
	od_ops.powersave_bias_target = generic_powersave_bias_target;
	od_set_powersave_bias(0);
}
EXPORT_SYMBOL_GPL(od_unregister_powersave_bias_handler_spsa2);
